...
/**
 * **Feature: dev-tools-dashboard, Property 2: Tool card click navigation**
 * **Validates: Requirements 2.1**
 *
 * Property: For any tool card, clicking it should trigger navigation to that tool's view with the correct tool ID
 */

describe("Property 2: Tool card click navigation", () => {
  it("should navigate to the correct tool when any tool card is clicked", () => {
    fc.assert(
      fc.property(
        ...
        // Generate which tool to click (index)
        fc.nat(),
        (tools: Tool[], clickIndex: number) => {
          if (tools.length === 0) return;

          const toolToClick = tools[clickIndex % tools.length];
          ...
          // Click the specific tool card
          const cardToClick = toolCards[
            clickIndex % tools.length
          ] as HTMLElement;
          ...
          expect(mockOnToolSelect).toHaveBeenCalledWith(toolToClick.id);
  ...
  it("should navigate to the correct tool for each card in a multi-tool setup", () => {
    fc.assert(
      fc.property(
  ...
  it("should navigate when clicking tool cards in pinned and recently used sections", () => {
    fc.assert(
      fc.property(
        // Generate tools
        fc
  ...
  it("should only call onToolSelect once per click", () => {
    fc.assert(
      fc.property(
        // Generate a list of tools
        fc
